...
export default function NuevoPaquetePage() {
  ...
  const [validezDesde, setValidezDesde] = useState<string>(
    formatStoredDate(new Date()) ?? "",
  );
  const [validezHasta, setValidezHasta] = useState<string>(
  ...
  const handleCreate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const newPaquete = await createPaquete({
        ...
        precioVenta: 0,
        moneda: "USD",
        validezDesde: validezDesde || now || "",
        validezHasta: validezHasta || oneYearLaterFormatted || "",
        ordenServicios: [],
      });
            ...
            <FieldGroup columns={2}>
              ...
              <Field span={2}>
                <PeriodPicker
                  ...
                  valueFrom={validezDesde}
                  valueTo={validezHasta}
                  onChange={(desde, hasta) => {
                    setValidezDesde(desde);
                    setValidezHasta(hasta);
                  }}

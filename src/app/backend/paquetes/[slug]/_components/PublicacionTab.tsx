...
export function PublicacionTab({ paquete }: PublicacionTabProps) {
  ...
  const handleValidezChange = (desde: string, hasta: string) => {
    ...
    updatePaquete({
      ...paquete,
      validezDesde: desde || paquete.validezDesde,
      validezHasta: hasta || paquete.validezHasta,
    });
    toast("success", "Periodo de validez actualizado");
  };
  ...
  return (
    <FormSections>
      ...
      >
        <FieldGroup>
          <Field>
            <FieldLabel>Periodo de validez</FieldLabel>
            <PeriodPicker
              valueFrom={paquete.validezDesde}
              valueTo={paquete.validezHasta}
